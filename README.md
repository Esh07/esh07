
<h1>Hi there 👋</h1>
<p>I'm a Computer Science graduate and software engineer from Leicester, UK. <em>Building practical, user-friendly tools that I'd actually want to use myself.</em> </p>

### 🔭 What I'm working on now
- __Python Accounting Simulator__ — CLI tool that generates realistic invoices, expenses, payroll, and bank statements for personal finance and small business testing.
- [AGMP - Advanced Geolocation Mapping Tool](https://advanced-map.vercel.app/docs) — Browser-based geospatial planner using Leaflet + OpenStreetMap. Supports boundary drawing, street/house analysis, team workload management, and local-first data processing with DuckDB-Wasm.
- Strengthening DSA fundamentals and system design through practice.


### 💼 Open to
Graduate **Software Engineering** roles where I can help build useful products that solve meaningful problems.

:inbox_tray: [esh(nospace)dev[mention]proton(full-stop)me](mailto:eshdev@proton.me)

Feel free to reach out if you're working on something interesting!
